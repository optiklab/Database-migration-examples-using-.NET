using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using Dapper;
using Npgsql;
using System.Data;

namespace Phi.Repository.Postgres.Dapper
{

    public class DbService : IDbService
    {
        private readonly IDbConnection _db;

        /// <summary>
        /// Ctr.
        /// </summary>
        /// <param name="connectionString">
        /// Calee must user IConfiguration to get config from config section:
        /// "ConnectionStrings": {
        ///     "Employeedb": Host=localhost;Port=5432;Database=testdb;User Id = *******; Password=*******;"
        /// }
        /// by calling config.GetConnectionString("Employeedb"));
        /// </param>
        public DbService(string connectionString)
        {
            _db = new NpgsqlConnection(connectionString);
        }

        public async Task<T> GetAsync<T>(string command, object parms)
        {
            T result;
            result = (await _db.QueryAsync<T>(command, parms).ConfigureAwait(false)).FirstOrDefault();
            return result;
        }

        public async Task<List<T>> GetAll<T>(string command, object parms)
        {
            List<T> result = new List<T>();
            result = (await _db.QueryAsync<T>(command, parms)).ToList();
            return result;
        }

        public async Task<T> Insert<T>(string command, object parms)
        {
            T result;
            result = _db.Query<T>(command, parms).FirstOrDefault();
            return result;
        }

        public async Task<T> Update<T>(string command, object parms)
        {
            T result;
            result = _db.Query<T>(command, parms).FirstOrDefault();
            return result;
        }

        public async Task<T> Delete<T>(string command, object parms)
        {
            T result;
            result = _db.Query<T>(command, parms).FirstOrDefault();
            return result;
        }
    }
}
